...
$code=$_GET['code'];
$sql='select * from exhibition where code like :code';
$pdo=get_db_connect();
  ...
  if(mb_strlen($str6)>1){
    ...
    if(strlen($str1)>4){
      ...
      for($i=0;$i<count($array1);$i++){
        ...
        $img_tag.='<div class="e-img"><img src="./image/exhibition/'.$code.'/'.$array1[$i].'" class="fitimg clickable" @click="clickimg='.$num.'"><p>'.$array2[$i].'</p></div><div v-show="clickimg=='.$num.'" class="popimgbg"><div class="popimg"><button @click="clickimg=0" class="popclose"><i class="fas fa-times"></i></button><img src="./image/exhibition/'.$code.'/'.$array1[$i].'" class="fitimg"></div><p class="poptext">'.$array2[$i].'</p></div>';
      }
    }
    ...
    <body>
      <div id="all">
        ...
        <div id="wrap">
          ...
          <div id="pnkz">
              <ul>
                ...
                <li><a href="/events/general/index.html">企画一覧</a></li>
          ...
          if(mb_strlen($form)>12){
            echo('<h2>ご質問・意見・感想</h2><p>本企画（'.$event_name.'）についての質問・意見・感想がある方は必要項目を記入の上、以下のフォームからお願いします。なお、回答には時間を要する場合があります。ご了承下さい。</p><div class="g-wrap"><div class="gForm">'.$form.'</div></div>');
          }else{
            echo('<h2>ご質問などについて</h2><p>本企画（'.$event_name.'）では質問などは受け付けておりません。ご了承下さい。</p>');
          }
           ?>           
        ...
        <footer-menu></footer-menu>
          ...
          <script>
            var event_name="<?php echo($event_name);?>";//イベントヘッダーの企画名
            var author1="<?php echo($group_name); ?>";//企画名
            var author2="";
